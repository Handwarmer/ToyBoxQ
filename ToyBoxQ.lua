...
function ToyBoxQ:UICustomTab(container, event, group)
    ...
    categoryHeader:SetText("Choose Category");
    categoryHeader:SetRelativeWidth(1);
    sf:AddChild(categoryHeader);

    local categorySelectorGroup = AceGUI:Create("SimpleGroup");
    categorySelectorGroup:SetLayout("Flow");
    categorySelectorGroup:SetFullWidth(true);

    local selectedCategoryIndex = nil;
    local selectedCategoryName = nil;
    if (TBQ.db.char.Categories == nil) then
        TBQ.db.char.Categories = {};
    end ;

    -- Category selector
    local categorySelector = AceGUI:Create("Dropdown");
    local ChangeCategory = nil;
    local ListToysUnderCategory = nil;

    ChangeCategory = function(dropdown, event, value)
        selectedCategoryIndex = value;
        selectedCategoryName = TBQ.db.char.Categories[value];
        ListToysUnderCategory();
    end

    --if not tContains(TBQ.db.char.Categories, "Create New") then
    --    tinsert(TBQ.db.char.Categories, "Create New");
    --end

    categorySelector:SetRelativeWidth(0.25);
    categorySelector:SetList(TBQ.db.char.Categories);
    categorySelector:SetCallback("OnValueChanged", ChangeCategory);

    categorySelectorGroup:AddChild(categorySelector);
    --Category selector end

    --Create Category button
    local function CreateNewCategoryPopup()
        StaticPopupDialogs["TBQ_CreateNewCategory"] = {
            ...
            OnAccept = function(self, data, data2)
                ...
                if (trimmedText ~= "" and not tContains(TBQ.db.char.Categories, text)) then
                    tinsert(TBQ.db.char.Categories, 1, text);
                    categorySelector:SetList(TBQ.db.char.Categories);
                    categorySelector:SetValue(1);
                    ChangeCategory(categorySelector, nil, 1);
                end;
            end,
        };
        dialog = StaticPopup_Show("TBQ_CreateNewCategory")
        if dialog then
            -- This makes sure the popup is always on top of other frames
            dialog:SetFrameStrata("TOOLTIP")
        end
    end

    local createCategoryButton = AceGUI:Create("Button");
    createCategoryButton:SetRelativeWidth(0.25);
    createCategoryButton:SetText("Create Category");
    createCategoryButton:SetDisabled(false);
    createCategoryButton:SetCallback("OnClick", CreateNewCategoryPopup);

    categorySelectorGroup:AddChild(createCategoryButton);
    --Create Category button end

    --Delete Category button
    local deleteCategoryButton = AceGUI:Create("Button");
    deleteCategoryButton:SetRelativeWidth(0.4);
    deleteCategoryButton:SetText("Delete Current Category");
    deleteCategoryButton:SetDisabled(false);
    deleteCategoryButton:SetCallback("OnClick", function(self, event, key)
        if (selectedCategoryIndex) then
            tremove(TBQ.db.char.Categories, selectedCategoryIndex);
            TBQ.db.char.CustomCategory[selectedCategoryName] = nil;
            selectedCategoryIndex = nil;
            selectedCategoryName = nil;
            categorySelector:SetList(TBQ.db.char.Categories);
            categorySelector:SetValue(nil);
        end
    end);

    categorySelectorGroup:AddChild(deleteCategoryButton);
    --Delete Category button end

    sf:AddChild(categorySelectorGroup);
    --Category selector group end

    local toyHeader = AceGUI:Create("Heading");
    toyHeader:SetText("Toys");
    toyHeader:SetRelativeWidth(1);
    sf:AddChild(toyHeader);

    local addToyBox = AceGUI:Create("EditBox");
    addToyBox:SetRelativeWidth(0.4);
    addToyBox:SetLabel("Enter (or drop) new item here:");
    addToyBox:SetCallback("OnEnterPressed", function(self, event, text)
        if text == "" then
            return
        end
        ...
        --if toy ~= nil then
        --    ToyBoxQ:Print("Item", itemName, "is already in this category");
        --    return;
        --end

        if tContains(TBQ.db.char.CustomCategory[selectedCategoryName], toyID) then
            ToyBoxQ:Print("Item", itemName, "is already in this category");
            return;
        end
        ...
        if TBQ.db.char.CustomCategory == nil then
            TBQ.db.char.CustomCategory = {}
        end ;
        if selectedCategoryName == nil then
            ToyBoxQ:Print("Please select one category first")
        end ;
        if TBQ.db.char.CustomCategory[selectedCategoryName] == nil then
            TBQ.db.char.CustomCategory[selectedCategoryName] = {}
        end ;
        table.insert(TBQ.db.char.CustomCategory[selectedCategoryName], toyID);
        ListToysUnderCategory();
    end);
    addToyBox:SetCallback("OnEnter", function(self, event)
        TBQ_AddTooltipInfo(self, "This allows you to add items from your collections and inventory to ToyBoxQ. Enter item name, id or drag and drop the item.");
    end);
    addToyBox:SetCallback("OnLeave", function()
        ShowUIPanel(GameTooltip)
        GameTooltip:Hide()
    end);
    sf:AddChild(addToyBox);

    local toys = AceGUI:Create("SimpleGroup");
    toys:SetLayout("Flow");
    toys:SetHeight(22);
    toys:SetFullWidth(true);
    sf:AddChild(toys);

    ListToysUnderCategory = function()
        toys:ReleaseChildren();
        for i, j in pairs(TBQ.db.char.CustomCategory[selectedCategoryName]) do
            local toyContainer = AceGUI:Create("SimpleGroup");
            toyContainer:SetLayout("Flow");
            toyContainer:SetHeight(20);
            toyContainer:SetRelativeWidth(0.3333);
            ...
            box:SetCallback("OnLeave", function()
                ShowUIPanel(GameTooltip)
                inTooltip = false;
                GameTooltip:Hide()
            end);
            toyContainer:AddChild(box);
            ...
            btn:SetCallback("OnClick", function(self, event, key)
                ...
                table.remove(TBQ.db.char.CustomCategory[selectedCategoryName], idx);
                ShowUIPanel(GameTooltip);
                GameTooltip:Hide();
                ListToysUnderCategory();
            end);
            ...
            toyContainer:AddChild(btn);

            toys:AddChild(toyContainer);
        end
    end
end
...
function ToyBoxQ:UpdateToyCnt()
    if TBQ.frame == nil then
        return
    end
    if TBQ.KnownOppFacCnt == nil then
        TBQ.KnownOppFacCnt = 0
    end
    local text
...
local function TBQ_DataStore_CheckCharacter(character, name, realm, account)
    ...
    if myrealm == realm and myname == name then
        return
    end
